...
public class FunEncoderVisitor extends AbstractParseTreeVisitor<Void> implements FunVisitor<Void> {
	...
	* @return the visitor result
	*/
	public Void visitActual(ActualseqContext ctx) {
	...
	@Override
	public Void visitDefault_case(Default_caseContext ctx) {
		visit(ctx.seq_com());
		return null;
	}
	...
	public Void visitCase_statement(Case_statementContext ctx) {
		...
		if(ctx.FALSE() != null){
			...
			int v1 = Integer.parseInt(ctx.n1.NUM().getText());
			obj.emit12(SVM.LOADC, value);
			int v2 = Integer.parseInt(ctx.n2.NUM().getText());
			obj.emit12(SVM.LOADC, value);
		}
