...
import ast.FunParser.Case_statementContext;
import ast.FunParser.Default_caseContext;
import ast.FunParser.ExprContext;
import ast.FunParser.ForloopContext;
import ast.FunParser.Formal_decl_seqContext;
import ast.FunParser.FormalseqContext;
import ast.FunParser.RangeContext;
...
public class FunCheckerVisitor extends AbstractParseTreeVisitor<Type> implements FunVisitor<Type> {
	...
	public Type visitSwitch(FunParser.SwitchContext ctx) {
		Type t = visit(ctx.expr());
		List<FunParser.case_stmtContext> cases = ctx.case_stmt();
		Set<String> guards = new HashSet<String>();
		for (FunParser.case_stmtContext c : cases) {          
			Type caseType = visit(c);
			checkType(t, caseType, ctx);
			String guard  = c.v1.getText();
			if (guards.contains(guard)) {
				reportError("Duplicate guard " + guard, ctx);
			}
			else {
				guards.add(guard);
			}
	 ...
	 */
	public Type visitdefault_case(FunParser.default_caseContext ctx) {
		visit(ctx.seq_com());
		return null;
	}  
	 ...
	 * @return the visitor result
	 */
	public Type visitActual(ActualseqContext ctx) {
		FunParser.ExprContext ec = (ExprContext) ctx.expr();
		Type t;
		if (ec != null) {
			t = visit(ec);
		}
	...
	@Override
	public Type visitFormalseq(FormalseqContext ctx) {
	...
	@Override
	public Type visitSwitch(SwitchContext ctx) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Type visitDefault_case(Default_caseContext ctx) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Type visitCase_statement(Case_statementContext ctx) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Type visitRange(RangeContext ctx) {
		// TODO Auto-generated method stub
		return null;
	}
